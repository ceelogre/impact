...
$str_query="SELECT DISTINCT project_id,name,description,target,prodate,public_key,image FROM project ORDER BY name";
$data=mysqli_query($mysqli,$str_query);

if($data){
	$mydataset=array();
	while($row=mysqli_fetch_assoc($data)){
		$mydataset[]=array('projectName'=>$row['name'],'projectDes'=>$row['description'],'protarget'=>$row['target'],'projdate'=>$row['prodate'],'prokey'=>$row['public_key'],'imagec'=>$row['image']);
	}
}
